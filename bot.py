...
# Блокировка рендера для каждого юзера от конфликтов печки и UI
SEND_VIEW_LOCKS = {}
def get_user_lock(uid):
    if uid not in SEND_VIEW_LOCKS:
        SEND_VIEW_LOCKS[uid] = asyncio.Lock()
    return SEND_VIEW_LOCKS[uid]
...
def get_block_aabbs(btype, bdata):
    if btype == "torch":
        dx, dy, dz = bdata.get("attach", (0,0,1))
        w, h = 0.08, 0.5
        if dx == 1:   cx, cy, cz = 0.1, 0.5, 0.25
        elif dx == -1: cx, cy, cz = 0.9, 0.5, 0.25
        elif dy == 1:  cx, cy, cz = 0.5, 0.1, 0.25
        elif dy == -1: cx, cy, cz = 0.5, 0.9, 0.25
        else:          cx, cy, cz = 0.5, 0.5, 0.0
        return [(cx-0.15, cy-0.15, cz, cx+0.15, cy+0.15, cz+0.6)]
    elif "slab" in btype:
        return [(0, 0, 0, 1, 1, 0.5)]
    elif "stairs" in btype:
        boxes = [(0, 0, 0, 1, 1, 0.5)]
        facing = bdata.get("facing", "front")
        if facing == "front": boxes.append((0, 0.5, 0.5, 1, 1, 1))
        elif facing == "back": boxes.append((0, 0, 0.5, 1, 0.5, 1))
        elif facing == "right": boxes.append((0, 0, 0.5, 0.5, 1, 1))
        elif facing == "left": boxes.append((0.5, 0, 0.5, 1, 1, 1))
        return boxes
    return [(0, 0, 0, 1, 1, 1)]
    ...
    def f(d):
        ...
        for y, row in enumerate(pixels):
            for x, char in enumerate(row):
                if char == "1":
                    d.rectangle((x*pw, y*ph, (x+1)*pw, (y+1)*ph), fill=(0,0,0))
                elif char == "2":
                    if mode == 2 or (mode == 1 and x < 3):
                        d.rectangle((x*pw, y*ph, (x+1)*pw, (y+1)*ph), fill=(255,50,50))
                    elif mode == 1 and x == 3:
                        d.rectangle((x*pw, y*ph, x*pw + pw//2, (y+1)*ph), fill=(255,50,50))
                        d.rectangle((x*pw + pw//2, y*ph, (x+1)*pw, (y+1)*ph), fill=(50,50,50))
                    else:
                        d.rectangle((x*pw, y*ph, (x+1)*pw, (y+1)*ph), fill=(50,50,50))
    return f
...
def draw_glass(d):
    d.rectangle((0,0,128,128), fill=(0,0,0,0))
    d.rectangle((0,0,127,127), outline=(200,255,255,255), width=8)
    d.line((16,16, 48,48), fill=(255,255,255,200), width=4)
...
create_fallback_tex("peschanik.png", (210, 190, 140))
...
create_fallback_tex("der_lopata.png", (0,0,0,0), get_shovel_drawer(c_wood), True)
create_fallback_tex("kam_lopata.png", (0,0,0,0), get_shovel_drawer(c_stone), True)
create_fallback_tex("zhel_lopata.png", (0,0,0,0), get_shovel_drawer(c_iron), True)
create_fallback_tex("alm_lopata.png", (0,0,0,0), get_shovel_drawer(c_diam), True)

create_fallback_tex("der_motiga.png", (0,0,0,0), get_hoe_drawer(c_wood), True)
create_fallback_tex("kam_motiga.png", (0,0,0,0), get_hoe_drawer(c_stone), True)
create_fallback_tex("zhel_motiga.png", (0,0,0,0), get_hoe_drawer(c_iron), True)
create_fallback_tex("alm_motiga.png", (0,0,0,0), get_hoe_drawer(c_diam), True)

create_fallback_tex("der_mech.png", (0,0,0,0), get_sword_drawer(c_wood), True)
create_fallback_tex("kam_mech.png", (0,0,0,0), get_sword_drawer(c_stone), True)
create_fallback_tex("zhel_mech.png", (0,0,0,0), get_sword_drawer(c_iron), True)
create_fallback_tex("alm_mech.png", (0,0,0,0), get_sword_drawer(c_diam), True)
...
create_fallback_tex("d_ugol.png", (0, 0, 0, 0), draw_coal, rgba=True)
...
create_fallback_tex("non_hp.png", (0,0,0,0), get_heart_drawer(0), True)
...
TEX_CACHE = {
    ...
    "sandstone": load_tex("peschanik.png", (210, 190, 140)),
    ...
    "wood_lopata": load_tex("der_lopata.png", (180, 140, 80)),
    "stone_lopata": load_tex("kam_lopata.png", (100, 100, 100)),
    "iron_lopata": load_tex("zhel_lopata.png", (220, 220, 220)),
    "diamond_lopata": load_tex("alm_lopata.png", (0, 255, 255)),
    "wood_motiga": load_tex("der_motiga.png", (180, 140, 80)),
    "stone_motiga": load_tex("kam_motiga.png", (100, 100, 100)),
    "iron_motiga": load_tex("zhel_motiga.png", (220, 220, 220)),
    "diamond_motiga": load_tex("alm_motiga.png", (0, 255, 255)),
    "wood_mech": load_tex("der_mech.png", (180, 140, 80)),
    "stone_mech": load_tex("kam_mech.png", (100, 100, 100)),
    "iron_mech": load_tex("zhel_mech.png", (220, 220, 220)),
    "diamond_mech": load_tex("alm_mech.png", (0, 255, 255)),
    ...
    "d_ugol": load_tex("d_ugol.png", (0,0,0,0)),
...
def get_inv_icon(itype):
    if itype not in INV_ICONS:
        ...
        is_stairs = "stairs" in itype
        
        if is_slab or is_stairs:
        ...
        base = TEX_CACHE.get(tex_name, TEX_CACHE.get(itype, TEX_CACHE["zemlya"]))
        
        if itype == "torch":
            t_base = base.resize((14, 28), Image.Resampling.NEAREST)
            icon = Image.new("RGBA", (28, 28), (0,0,0,0))
            icon.paste(t_base, (7, 0), t_base) 
            INV_ICONS[itype] = icon
        elif is_slab:
            icon = Image.new("RGBA", (128, 128), (0,0,0,0))
            half = base.crop((0, 64, 128, 128))
            icon.paste(half, (0, 64))
            INV_ICONS[itype] = icon.resize((28, 28), Image.Resampling.NEAREST)
        elif is_stairs:
            icon = Image.new("RGBA", (128, 128), (0,0,0,0))
            bottom = base.crop((0, 64, 128, 128))
            back = base.crop((64, 0, 128, 64))
            icon.paste(bottom, (0, 64))
            icon.paste(back, (64, 0))
            INV_ICONS[itype] = icon.resize((28, 28), Image.Resampling.NEAREST)
        else:
            INV_ICONS[itype] = base.resize((28, 28), Image.Resampling.NEAREST)
...
def get_body_front_tex(color, item_type):
    ...
    if cache_key not in HELD_ITEM_TEX_CACHE:
        ...
        if item_type:
            ...
            # Иконка строго по центру груди
            img.paste(icon, (64 - new_w//2, 40 - new_h//2), icon)
    ...
    def generate_chunk(self, cx, cy):
        ...
        for x in range(cx * 16, cx * 16 + 16):
            for y in range(cy * 16, cy * 16 + 16):
                ...
                if is_desert:
                    sand_depth = random.randint(2, 5)
                    for dz in range(sand_depth): 
                        self.blocks[(x, y, h-dz)] = {"type": "sand"}
                    self.blocks[(x, y, h-sand_depth)] = {"type": "sandstone"}
                    rock_start = h - sand_depth - 1
                else:
                ...
                for z in range(rock_start, -34, -1):
                    if z < -5:
                ...
                if not is_desert and 2 < (x%16) < 14 and 2 < (y%16) < 14 and random.random() < 0.02:
            ...
            for pos, b in list(srv.blocks.items()):
                if b.get("type") == "sand":
                    ...
                    if under not in srv.blocks or srv.blocks[under].get("type") == "torch":
                        if pos[2] > -34:
                            falls.append((pos, under))
            
            for old_pos, new_pos in falls:
                if old_pos in srv.blocks:
                    bdata = srv.blocks.pop(old_pos)
                    srv.blocks[new_pos] = bdata
...
async def furnace_ticker():
    while True:
        ...
        for s_id, srv in SERVERS.items():
            ...
            for pos, b in list(srv.blocks.items()):
                if b.get("type") == "furnace":
                    ...
                    i_in = inv[0]["type"] if inv[0] else None
                    out_type = None
                    if i_in == "iron_ore": out_type = "iron_ingot"
                    elif i_in == "wood": out_type = "d_ugol"
                    elif i_in == "cobblestone": out_type = "stone"
                    elif i_in == "sand": out_type = "glass"

                    can_smelt = inv[0] and out_type and (not inv[2] or (inv[2]["type"] == out_type and inv[2]["count"] < 64))
                    ...
                    if can_smelt and b.get("burn_time", 0) > 0:
                        ...
                        if b["smelt_time"] >= 5:
                            ...
                            else: inv[2] = {"type": out_type, "count": 1}
                    else:
                        b["smelt_time"] = 0
        ...
        for s_id in changed_srvs:
            for p_uid, ps in SERVERS[s_id].players.items():
                if ps.get("online") and ps.get("inv_mode") != "furnace":
                    asyncio.create_task(send_view(p_uid, p_uid))
...
def make_keyboard(uid):
    ...
    jump_text = "🦘 Прыгай!" if st.get("jump") else "🦘 Прыжок"
    ...
    kb.row(
        ...
        InlineKeyboardButton("↩️15°", callback_data="turn_l_15"),
        InlineKeyboardButton("↪️15°", callback_data="turn_r_15"),
        InlineKeyboardButton("▶️30°", callback_data="turn_r_30"),
        InlineKeyboardButton("⏩90°", callback_data="turn_r_90")
    )
    kb.row(
        InlineKeyboardButton("⏫90°", callback_data="look_up_90"),
        ...
        InlineKeyboardButton("⏬90°", callback_data="look_down_90")
    )
...
def get_keyboard(uid):
    st = get_st(uid)
    if not st: return InlineKeyboardMarkup()
    if st.get("inv_open"):
        kb = InlineKeyboardMarkup(row_width=3)
...
def draw_inv(img, d, w, h, st, srv=None):
    ...
    for sid, (sx, sy) in slots.items():
        ...
        elif 50 <= sid <= 52 and srv and st["furnace_pos"] in srv.blocks:
            item = srv.blocks[st["furnace_pos"]].get("inv", {}).get(sid - 50)
        elif 60 <= sid <= 74 and srv and st.get("chest_pos") in srv.blocks:
            item = srv.blocks[st["chest_pos"]].get("inv", {}).get(sid - 60)
...
def update_crafting(st):
    ...
    elif sg == [["sand", "sand"], ["sand", "sand"]]: res = {"type": "sandstone", "count": 4}
    ...
    elif sg == [["planks", "planks", "planks"], ["planks", None, "planks"], ["planks", "planks", "planks"]]: 
        res = {"type": "chest", "count": 1}
    ...
    elif sg == [["cobblestone", "cobblestone", "cobblestone"]]: res = {"type": "cobblestone_slab", "count": 6}
    elif sg == [["stone", "stone", "stone"]]: res = {"type": "stone_slab", "count": 6}

    # Ступеньки
    elif sg in (
        [["planks", None, None], ["planks", "planks", None], ["planks", "planks", "planks"]],
        [[None, None, "planks"], [None, "planks", "planks"], ["planks", "planks", "planks"]]
    ): res = {"type": "planks_stairs", "count": 4}
    elif sg in (
        [["cobblestone", None, None], ["cobblestone", "cobblestone", None], ["cobblestone", "cobblestone", "cobblestone"]],
        [[None, None, "cobblestone"], [None, "cobblestone", "cobblestone"], ["cobblestone", "cobblestone", "cobblestone"]]
    ): res = {"type": "cobblestone_stairs", "count": 4}
    elif sg in (
...
def get_slot_item(st, srv, idx):
    if idx < 50: return st["inv"].get(idx)
        ...
        b = srv.blocks.get(st.get("chest_pos"))
        if b and b.get("type") == "chest": return b.get("inv", {}).get(idx-60)
    return None
...
def set_slot_item(st, srv, idx, item):
    if idx < 50:
        ...
        b = srv.blocks.get(st.get("chest_pos"))
        if b and b.get("type") == "chest":
            if "inv" not in b: b["inv"] = {i:None for i in range(15)}
            b["inv"][idx-60] = item
...
def render_scene(px, py, pz, pa, pt, uid, s_id):
    ...
    for i in range(5):
        ...
        htex = tex.resize((14, 14), Image.Resampling.NEAREST)
        img.paste(htex, (hx, hy), htex)
...
def ray_pick(px, py, pz, pa, pt, s_id, ignore_uid=None):
    ...
    while t <= RAY_MAX_DIST:
        ...
        if cb in srv.blocks:
            ...
            btype = b.get("type", "")
            bx, by, bz = cb
            aabbs = get_block_aabbs(btype, b)
            hit = False
            for minx, miny, minz, maxx, maxy, maxz in aabbs:
                if (bx+minx) <= wx <= (bx+maxx) and (by+miny) <= wy <= (by+maxy) and (bz+minz) <= wz <= (bz+maxz):
                    hit = True
                    break
...
async def broadcast_chat(s_id, txt):
    ...
    for uid, st in list(SERVERS[s_id].players.items()):
        if st.get("msg_id") and st.get("online"):
            try: await bot.edit_message_caption(caption=cap, chat_id=uid, message_id=st["msg_id"], reply_markup=get_keyboard(uid))
            except Exception: pass
...
async def send_view(cid, uid):
    ...
    # Блокировка от конфликтов
    u_lock = get_user_lock(uid)
    if u_lock.locked(): return 
...
async def h_creative(m):
    ...
    if len(parts) > 1:
        if parts[1].lower() == "on":
            ...
            st["creative"] = False
            # Автоматическое падение на землю при отключении
            st["z"] = get_ground_z(st["x"], st["y"], SERVERS[s_id], st["z"])
...
async def h_cb(c):
    ...
    try:
        ...
        if st.get("inv_open"):
            ...
            if d in ("inv_click", "inv_click_1", "inv_u", "inv_d", "inv_l", "inv_r"):
                if mode in ("chest", "furnace"):
                    c_pos = st.get("chest_pos") if mode == "chest" else st.get("furnace_pos")
                    for p_uid, ps in srv.players.items():
        ...
        if d in ["move_f", "move_b", "move_l", "move_r", "move_fl", "move_fr", "move_bl", "move_br"]:
            ...
            if st.get("creative"):
                ...
                if srv.type == "survival":
                    # Бесконечная генерация привязана к дальности прорисовки
                    load_radius = max(1, st.get("view_radius", 8) // 16 + 1)
                    srv.load_chunks_around(nx, ny, radius=load_radius)
            ...
            if st.get("jump") and not st.get("creative"):
                ...
                if pb[0] == "block":
                    ...
                    if srv.blocks.get(pb[1], {}).get("type") == "bedrock": pass 
                        ...
                        if base_type in ("stone", "cobblestone", "coal_ore", "iron_ore", "diamond_ore", "furnace"):
                            ...
                            else: mhp = 6 if base_type == "wood" else 5
                        elif base_type in ("grass", "dirt", "sand"):
                            if t_type in ("iron_lopata", "diamond_lopata"): mhp = 1
                            elif t_type in ("wood_lopata", "stone_lopata"): mhp = 2
                            else: mhp = 3
                        elif base_type == "leaves":
                            if t_type in HOES: mhp = 1
                            else: mhp = 2
                        else:
                        ...
                        if srv.block_damage[pb[1]] >= mhp:
                            if btype == "furnace":
                                ...
                                c_inv = srv.blocks[pb[1]].get("inv", {})
                                for itm in c_inv.values():
                                    if itm:
                                        for i in range(20):
                                            if i not in st["inv"]:
                                                st["inv"][i] = itm; break
                    ...
                    if t_type == "diamond_mech": damage = 6
                    elif t_type == "iron_mech": damage = 5
                    elif t_type == "stone_mech": damage = 4
                    elif t_type == "wood_mech": damage = 3
                    elif t_type == "diamond_axe": damage = 4
                    elif t_type == "iron_axe": damage = 4
                    ...
                    async def reset_hit_btn(cid, p_uid, m_id):
                        ...
                        if p_st and p_st.get("msg_id") == m_id:
                            ...
                            try: await bot.edit_message_reply_markup(chat_id=cid, message_id=m_id, reply_markup=get_keyboard(p_uid))
                            except: pass
...
async def main():
    ...
    asyncio.create_task(physics_ticker())
    print("Bot running!")
